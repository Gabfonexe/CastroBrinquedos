                ...
                if users:            
                    for user in users:
                        for product in user.products:
                            ...
                            exist_date_unavailable = db.session.query(DateUnavailable).filter(and_(DateUnavailable.date == user.date, DateUnavailable.product == exist_product.type.value)).first()
                            if exist_product:
                                if exist_product.is_available == True:
                                    exist_product.quantity -= int(product['selectedQuantity'])
...
def date_available_routine(user:Users):
    ...
    if  difference_days <= -1: #TODO SE FOR INFLAVEL, PODE SER <= 0, verificar isso ainda.
        for product in user.products:
            ...
            if exist_product:
                exist_product.quantity += int(product['selectedQuantity'])
                exist_product.lost_quantity -= int(product['selectedQuantity'])
